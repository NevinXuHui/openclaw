...
捆绑的 Microsoft 语音提供商当前通过 `node-edge-tts` 库使用 Microsoft Edge 的在线神经 TTS 服务。这是一个托管服务(非本地),使用 Microsoft 端点,不需要 API 密钥。`node-edge-tts` 公开语音配置选项和输出格式,但并非所有选项都受服务支持。使用 `edge` 的旧配置和指令输入仍然有效,并被规范化为 `microsoft`。

因为这个路径是一个没有发布 SLA 或配额的公共 Web 服务,请将其视为尽力而为。如果您需要保证的限制和支持,请使用 OpenAI 或 ElevenLabs。
...
如果配置了多个提供商,首先使用选定的提供商,其他提供商作为备用选项。
...
完整架构在 [Gateway 配置](/gateway/configuration)中。
...
- `provider`: 语音提供商 id,如 `"elevenlabs"`、`"microsoft"` 或 `"openai"`(备用是自动的)。
- 如果 `provider` **未设置**,OpenClaw 按注册表自动选择顺序使用第一个配置的语音提供商。
...
- `modelOverrides`: 允许模型发出 TTS 指令(默认开启)。
  - `allowProvider` 默认为 `false`(提供商切换是选择加入的)。
- `providers.<id>`: 按语音提供商 id 键入的提供商拥有的设置。
...
- `providers.microsoft.outputFormat`: Microsoft 输出格式(例如 `audio-24khz-48kbitrate-mono-mp3`)。
  - 有关有效值,请参阅 Microsoft 语音输出格式;并非所有格式都受捆绑的 Edge 支持的传输支持。
...
加上可选的 `[[tts:text]]...[[/tts:text]]` 块来提供表达性标签(笑声、唱歌提示等),
这些标签应该只出现在音频中。

除非 `modelOverrides.allowProvider: true`,否则忽略 `provider=...` 指令。
...
- **Microsoft**: 使用 `microsoft.outputFormat`(默认 `audio-24khz-48kbitrate-mono-mp3`)。
  ...
  - Telegram `sendVoice` 接受 OGG/MP3/M4A;如果需要保证的 Opus 语音消息,请使用 OpenAI/ElevenLabs。
...
Discord 注意事项: `/tts` 是内置的 Discord 命令,因此 OpenClaw 在那里注册 `/voice` 作为原生命令。文本 `/tts ...` 仍然有效。
...
- `/tts status` 包括最新尝试的备用可见性:
  - 成功备用: `Fallback: <primary> -> <used>` 加上 `Attempts: ...`
  - 失败: `Error: ...` 加上 `Attempts: ...`
  - 详细诊断: `Attempt details: provider:outcome(reasonCode) latency`
- OpenAI 和 ElevenLabs API 失败现在包括解析的提供商错误详细信息和请求 id(当提供商返回时),这在 TTS 错误/日志中显示。

## Agent 工具

`tts` 工具将文本转换为语音,并返回用于回复传递的音频附件。当频道是 Feishu、Matrix、Telegram 或 WhatsApp 时,音频作为语音消息而不是文件附件传递。
